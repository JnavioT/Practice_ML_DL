...
if __name__ == "__main__":
    ...
    train_dataset = dataset.ClassificationDataset( image_paths=train_images, targets=train_targets, resize=(512, 512), augmentations=aug, )
    # torch dataloader creates batches of data
    # from classification dataset class
    train_loader = torch.utils.data.DataLoader( train_dataset, batch_size=16, shuffle=True, num_workers=4 )
    # same for validation data
    valid_dataset = dataset.ClassificationDataset( image_paths=valid_images,     targets=valid_targets, resize=(512, 512), augmentations=aug, )
    ...
    for epoch in range(epochs):
        ...
        # AlexNet:
        # Epoch=0, Valid ROC AUC=0.5864448190076417
        # Epoch=1, Valid ROC AUC=0.5839558195505782
        # Epoch=2, Valid ROC AUC=0.6435070460199508
        # Epoch=3, Valid ROC AUC=0.6134121536753563
        # Epoch=4, Valid ROC AUC=0.424475093798368
        # Epoch=5, Valid ROC AUC=0.4649425054496244
        # Epoch=6, Valid ROC AUC=0.5180295454729625
        # Epoch=7, Valid ROC AUC=0.540377462460394

        # ResNet
        # Epoch=0, Valid ROC AUC=0.6932627245690948
